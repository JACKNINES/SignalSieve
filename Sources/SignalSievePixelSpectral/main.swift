...
@main
enum SignalSievePixelSpectralCLI {
    ...
    static func main() {
        do {
            ...
            case .score(let input):
                let report = try PixelSpectralForensics.analyze(readBounded(input))
                try emit([
                    "schemaVersion": 1,
                    "detector": "Signal Sieve Spectral Carrier Forensics v1",
                    "score": report.score,
                    "threshold": report.threshold,
                    "label": report.hasEnoughSamples
                        ? (report.isElevated
                            ? "elevated periodic pixel carrier"
                            : "no elevated periodic pixel carrier")
                        : "insufficient pixel sample"
                ])
            case .regenerate(let input, let output, let strength):
                let regenerated = try PixelSpectralForensics.regenerate(
                    readBounded(input),
                    strength: strength
                )
                ...
                try emit([
                    "schemaVersion": 1,
                    "status": "ok",
                    "method": "bounded spectral carrier projection"
                ])
            }
        } catch {
            FileHandle.standardError.write(Data("Pixel spectral module failed: \(error)\n".utf8))
