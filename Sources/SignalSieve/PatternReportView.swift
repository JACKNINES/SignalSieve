        ...
        VStack(spacing: 0) {
            HStack {
                VStack(alignment: .leading, spacing: 3) {
                    Text(localized("Pattern Memory")).font(.title2.weight(.semibold))
                    Text(localized("Session-only analysis · nothing is saved to disk"))
                        .font(.subheadline)
                ...
                Text(formatted("%d samples", sampleCount))
            ...
            HStack {
                Text(localized("Correlation is not proof of authorship, AI generation, or watermarking."))
                    .font(.caption)
                    .foregroundStyle(.secondary)
                Spacer()
